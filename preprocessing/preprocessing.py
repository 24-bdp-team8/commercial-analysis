...
def preprocess_data(df):
    ...
    df = df.withColumn(
        "상권상태",
        when((col("위도") > 37.5) & (col("경도") > 126.9), lit("중심상권"))
        .otherwise(lit("기타"))
    )
...
def main():
    ...
    try:
        ...
        # HDFS에 전처리 데이터 저장
        processed_df.coalesce(1).write.mode("overwrite").parquet(f"{HDFS_OUTPUT_DIR}/preprocessed_data.parquet")
